...
class Grid():
    ...
    def get(self, n, m, end_grid):
        ...
        end_grid= self.__list_to_tuple(end_grid)
        solution = [{start_grid:0}]
        
        while True:
            top = list(solution[-1].keys())[0]
            number = solution[-1][top]
            empty_cell = self.__get_first_empty_cell(top, end_grid)
            if empty_cell == None:
                return solution[-1]
            x=solution.pop(-1)
            for triomino in self.triomino_list:
                next_grid = self.__place_triomino(top, triomino, empty_cell,end_grid, number)
                if not next_grid:
                    continue
                solution.append({next_grid: number+1})
            
    def __get_first_empty_cell(self, grid, end_grid):
    ...
    def __place_triomino(self, grid, triomino, empty_cell,end_grid, number):
...
def tileRec(n, x, y):
    ...
    tileRec(int(n / 2), x, y + int(n / 2))
    tileRec(int(n / 2), x, y)
    tileRec(int(n / 2), x + int(n / 2), y)
    tileRec(int(n / 2), x + int(n / 2), y + int(n / 2))
...
def solvable(r,c,n):
    ...
    if (r==n-2 and c==1) or (r==n-3 and c==0) or (r==n-3 and c ==2) or (r==n-4 and c==1):
        return False
    if (r==n-2 and c==n-3) or (r==n-3 and c==n-2) or (r==n-3 and c ==n-4) or (r==n-4 and c==n-2):
        return False
...
def tile(n, x, y):
    ...
    if n == 8:
        ...
        arr[x:x+n,y:y+n] = np.array(list(Grid().get(8,8,end_grid).keys())[0])  
        cnt+=20
        return 
    ...
    if(r < x + int(n / 2) - 1 and c == y + int(n / 2) - 1):
        ...
        place(x  + int(n / 2) + int(n/4) - 1, y+ int(n / 4), x + int(n / 2) + int(n/4), y + int(n / 4), x + int(n / 2) + int(n/4), y + int(n / 4) - 1)
        tileRec(int(n / 4), x + int(n / 4)+ int(n / 2), y  + int(n/4))
        tileRec(int(n / 4), x+ int(n / 2) + int(n/4), y )
        tileRec(int(n / 4), x + int(n / 2), y + int(n / 4))
              
        place(x+int(n/2)+int(n/4)-1, y+int(n/2)+int(n/4)-1, x+int(n/2)+int(n/4), y+int(n/2)+int(n/4)-1, x+int(n/2)+int(n/4), y+int(n/2)+int(n/4))
        tileRec(int(n / 4), x + int(n / 2), y + int(n / 2))
        tileRec(int(n / 4), x + int(n / 2) + int(n/4), y + int(n / 2))
        tileRec(int(n / 4), x + int(n / 2) + int(n/4), y  + int(n / 2) + int(n/4))
    
    elif(r > x + int(n / 2) - 1 and c == y + int(n / 2) - 1):
        arr[x:x+n,y:y+n] = np.flipud(arr[x:x+n,y:y+n])
        tile(n, x , y)
        arr[x:x+n,y:y+n] = np.flipud(arr[x:x+n,y:y+n])
        
    elif(r == x + int(n / 2) - 1 and c > y + int(n / 2) - 1):
        arr[x:x+n,y:y+n] = np.rot90(arr[x:x+n,y:y+n])
        tile(n, x , y)
        arr[x:x+n,y:y+n] = np.rot90(arr[x:x+n,y:y+n],3) 
